...
class Battery(object):
    ...
    def __init__(self, batterycontrol: Battery_Control) -> None:
        super().__init__()
        rospy.init_node('battery_guı', anonymous=True)
        self.battery_publisher = rospy.Publisher(
            '/battery_status', Batterystatus, queue_size=10)  # publisher for battery status
        self.battery_subscriber = rospy.Subscriber(
            '/battery_status', Batterystatus, self.battery_callback)  # subscriber for battery status
    ...
    def publish_ones(self) -> None:
        ...
        # battery status (1: charging, 0: discharging, 2: idle, 3: error)
        self.batterystatu.battery_status = self.batterystatu.POWER_SUPPLY_STATUS_IDLE
        ...
        while not self.ctrl_c:
            ...
            if connects > 0:
                self.battery_publisher.publish(
                    self.batterystatu)  # PUBLISH BATTERY STATUS
...
if __name__ == '__main__':
    try:
        ...
        if len(sys.argv) > 3:
            ...
            # CREATE OBJECT OF BATTERY CONTROL CLASS
            battery_control = Battery_Control(port, slave_address, debug)
            # CREATE OBJECT OF BATTERY CLASS
            battery = Battery(battery_control)
