...
class Coordinates:
    ...
    def print_coordinates(self):
        rospy.loginfo("[ x, y, z] = [" + str(self.x) + ", " + str(self.y) + ", " + str(self.z) + "]")
        rospy.loginfo("[ roll, pitch, yaw] = [" + str(self.roll) + ", " + str(self.pitch) + ", " + str(self.yaw) + "]")
...
class MoveModel(object):
    ...
    def calculate_coord_for_speed(self, publish_rate):
        """
        ...
        if pose_now:
            ...
            explicit_quat = [pose_now.orientation.x, pose_now.orientation.y, pose_now.orientation.z, pose_now.orientation.w]
            pose_now_euler = tf.transformations.euler_from_quaternion(explicit_quat)
            ...
            planar_increment = self.current_cmd_speed.linear.x * (1.0/publish_rate)
            x_yaw_comp = math.cos(yaw)
            y_yaw_comp = math.sin(yaw)
            
            elevation_increment = self.current_cmd_speed.linear.z * (1.0/publish_rate)
